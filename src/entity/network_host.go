...
func (n *NetworkHost) UnmarshalYAML(unmarshal func(interface{}) error) error {
	...
	if shellresolve.IsShell(temp.Password) {
		l("!!! shell resolve case found")
		var e error
		temp.Password, e = shellresolve.ResolveShell(temp.Password)
		if e != nil {
			return e
		}
	}
...
func checkHostsForm(host string) NetworkHost {
	...
	// check if password is shell resolve case
	if shellresolve.IsShell(result.Password) {
		pass, e := shellresolve.ResolveShell(result.Password)
		if e != nil {
	...
	l(spew.Sdump("dump: 3DB74440-E5D9-4BEE-89D8-9C4EEB1459A9", result))
