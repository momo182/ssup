...
// func isCommand(conf *entity.Supfile, singleArgument string) (entity.Command, error) {
// 	l := kemba.New("usecase::ParseInitialArgs::checkCommands").Printf
// 	l("check if its a command")
// 	command, isCommand := conf.Commands.Get(singleArgument)
// 	if !isCommand {
// 		return entity.Command{}, fmt.Errorf("%v: %v", entity.ErrCmd, singleArgument)
// 	}
// 	return command, nil
// }
...
// func allArgsAreCommands(conf *entity.Supfile, args []string, helpMenu entity.HelpDisplayer) bool {
// 	l := kemba.New("usecase::ParseInitialArgs::allArgsAreCommands").Printf
// 	noMissingNames := true
// 	l("check if all given args are commands: %v", len(args))
// 	for _, cmd := range args {
// 		if !conf.Commands.Has(cmd) {
// 			l("commands check -> unknown keyword: %v", cmd)
// 			// helpMenu.ShowAll(conf)
// 			// os.Exit(26)
// 		}
// 	}
